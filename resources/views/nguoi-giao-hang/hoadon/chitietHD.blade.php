            ...
            <div class="row g-gs">
                ...
                <div class="col-lg-12" style="margin-left: 500px;">
                    <span style="float: left">
                        <form action="{{route('xac-nhan-giao-hang')}}" method="POST">
                            {{csrf_field()}}
                            <input type="hidden" name="idHD" value="{{$ct->id}}">
                               <td><button type="submit" class="btn btn-warning">Nhận đơn</button></td>
                        </form>
                    </span>
                    @if($ct->id_TT == 3)
                        <span >
                            <form action="{{route('tu-choi-giao-hang')}}" method="POST">
                            {{csrf_field()}}
                            <input type="hidden" name="idHD" value="{{$ct->id}}">
                               <td><button type="submit"  class="btn btn-danger" style="margin-left: 10px;">Từ chối</button></td>
                            </form>
                        </span>
                    @elseif($ct->id_TT == 2)
                        <span >
                         <td><button type="submit"  class="btn btn-danger" style="margin-left: 10px;pointer-events: none;">Từ chối</button></td>
                        </span>
                    @endif
                </div>
                @endforeach
